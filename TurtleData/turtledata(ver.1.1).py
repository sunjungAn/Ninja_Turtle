...
f = open('turtledata.csv','w', newline='')
wr = csv.writer(f)
wr.writerow(['nose length', 'right_ear length', 'left_ear length'])
...
with mp_pose.Pose(
  ...
  while cap.isOpened():
    ...
    cv2.putText(image, "nose length: {}".format(nose_length),(10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
    cv2.putText(image, "right_ear length: {}".format(rear_length),(10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
    cv2.putText(image, "left_ear length: {}".format(lear_length),(10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)   
    cv2.imshow('NinjaTurtle(ver.1.1)', image)
    wr.writerow([nose_length, rear_length, lear_length])
